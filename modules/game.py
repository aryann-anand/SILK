...
def record():
	...
	with sr.Microphone() as source:
		...
		try:
			...
			print(e)
			speak("Bruh, that move’s kinda sus, not gonna lie…")
			return "None"	
	return said.lower()
...
class RockPaperScissor:
	...
	def intro(self):
		speak("Starting the rock paper scissor game. To STOP the Match, say STOP or Cancel.")
	...
	def whoWon(self):
		...
		if self.playerScore == self.botScore:
			result = "It's a tie, fam!\n"
		elif self.playerScore > self.botScore:
			result = "You clutched it, boss! GG!\n"
		else:
			result = "You took the L, boss! Haha!\n"
		...
		result += "You snagged the W! " +str(self.playerScore)+"/"+str(self.total_moves)+" times."
...
def rockPaperScissorWindow():
	...
	rockImg = ImageTk.PhotoImage(Image.open('assets/image/rps/1.jpg'))
	paperImg = ImageTk.PhotoImage(Image.open('assets/image/rps/2.jpg'))
	scissorImg = ImageTk.PhotoImage(Image.open('assets/image/rps/3.jpg'))
	grayImg = ImageTk.PhotoImage(Image.open('assets/image/rps/grayQuestion.png'))
	orangeImg = ImageTk.PhotoImage(Image.open('assets/image/rps/orangeQuestion.jpg'))
	winImg = ImageTk.PhotoImage(Image.open('assets/image/rps/win.jpg'))
	loseImg = ImageTk.PhotoImage(Image.open('assets/image/rps/lose.jpg'))
	drawImg = ImageTk.PhotoImage(Image.open('assets/image/rps/draw.jpg'))
	...
	totalLabel.pack()
	#bottom image
	img = ImageTk.PhotoImage(Image.open('assets/image/rps/rockPaperScissor.jpg'))
...
def play(gameName):
	...
	if isContain(gameName, ['dice','die']):
		...
		p = randint(-10,10)
		if p>0: return "It's head"
		else: return "It's tail"
