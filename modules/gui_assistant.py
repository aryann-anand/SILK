	...
	with sr.Microphone() as source:
		...
		try:
			...
			if "connection failed" in str(e):
				speak("Systems offline...", True, True)
			return 'None'
	return said.lower()
...
def voiceMedium():
	while True:
		...
		if isContain(query, EXIT_COMMANDS):
			speak("Systems shut down. Adios "+ownerDesignation+"!", True, True)
			break
...
def main(text):
		...
		if "project" in text:
			if isContain(text, ['make', 'create']):
				speak("How would you name this project?", True, True)
		...
		if "translate" in text:
			speak("What're you tryna translate?", True, True)
			sentence = record(False, False)
			speak("Which language you wanna translate to?", True)
			langauage = record(False, False)
			result = normal_chat.lang_translate(sentence, langauage)
			if result=="None": speak("This language don't exist bruh!")
			else:
				speak(f"In {langauage.capitalize()} you wanna say:", True)
				if langauage=="hindi":
		...
		if 'list' in text:
			if isContain(text, ['add', 'create', 'make']):
				speak("What you wanna add?", True, True)
		...
		if 'selfie' in text or ('click' in text and 'photo' in text):
			...
			clickPhoto()
			speak('You wanna view your clicked photo?', True)
			query = record(False)
			if isContain(query, ['yes', 'sure', 'yeah', 'show me']):
				Thread(target=viewPhoto).start()
				speak("Alright, here’s the deets...", True, True)
			else:
				speak("No Problemo amigo "+ownerDesignation, True, True)
			return

		if 'volume' in text:
			app_control.volumeControl(text)
			Label(chat_frame, image=botIcon, bg=chatBgColor).pack(anchor='w',pady=0)		
			attachTOframe('Volume changed', True)
			return
		...
		if 'whatsapp' in text:
			speak("Sure "+ownerDesignation+"...", True, True)
			speak('Whom do you want to send the text to?', True)
			...
			speak("Text is on the way. Do not move away from the screen.")
			attachTOframe("Text Sent", True)
			return
		...
		if isContain(text, ['youtube','video']):
			speak("Ok "+ownerDesignation+", here's a video for you...", True, True)
			try:
				speak(web_scrapping.youtube(text), True)
			except Exception as e:
				print(e)
				speak("Searched query not returned", True)
			return
		...
		if isContain(text, ['map', 'direction']):
			if "direction" in text:
				speak("What's the starting location?", True, True)
				startingPoint = record(False, False)
				speak("Ok "+ownerDesignation+", Where do you wanna go to?", True)
				...
				try:
					...
					speak("I think location is not correct, Try Again!")
			else:
		...
		if isContain(text, ['news']):
			...
			speak('Do you wanna read full news?', True)
			text = record(False, False)
			if isContain(text, ["no","don't"]):
				speak("No Problemo "+ownerDesignation, True)
			else:
		...
		if isContain(text, ['screenshot']):
			...
			speak("Screenshot saved", True, True)
			return
		...
		if isContain(text, ['game']):
			speak("What game you wanna play?", True, True)
...
'''
def deleteUserData():
	result = messagebox.askquestion('Alert', 'Please confirm that you wanna delete your face data?')
	if result=='no': return
	messagebox.showinfo('Clear Face Data', 'Your face has been cleared from SILK systems!\nRegister your face again to use.')
	import shutil
...
def showSingleImage(type, data=None):
	...
	try:
		...
		pass
	img1 = ImageTk.PhotoImage(Image.open('assets/image/heads.jpg').resize((220,200), Image.ANTIALIAS))
	img2 = ImageTk.PhotoImage(Image.open('assets/image/tails.jpg').resize((220,200), Image.ANTIALIAS))
	img4 = ImageTk.PhotoImage(Image.open('assets/image/WeatherImage.png'))
	...
	if type=="weather":
		...
		img3 = ImageTk.PhotoImage(Image.open('assets/image/dice/'+type+'.jpg').resize((200,200), Image.ANTIALIAS))
...
def WAEMPOPUP(Service='None', rec='Reciever'):
	...
	if Service=="WhatsApp": PopUProot.iconbitmap("assets/image/whatsapp.ico")
...
def UpdateIMAGE():
	...
	userProfileImg = ImageTk.PhotoImage(Image.open("assets/image/avatar/a"+str(ownerPhoto)+".png").resize((120, 120)))

	userPhoto['image'] = userProfileImg
	userIcon = PhotoImage(file="assets/image/avatar/icons/a"+str(ownerPhoto)+".png")
...
if __name__ == '__main__':
	...
	cblLightImg = PhotoImage(file='assets/image/centralButton.png')
	cblDarkImg = PhotoImage(file='assets/image/centralButton1.png')
	...
	#Settings Button
	sphLight = PhotoImage(file = "assets/image/setting.png")
	sphLight = sphLight.subsample(2,2)
	sphDark = PhotoImage(file = "assets/image/setting1.png")
	...
	#Keyboard Button
	kbphLight = PhotoImage(file = "assets/image/keyboard.png")
	kbphLight = kbphLight.subsample(2,2)
	kbphDark = PhotoImage(file = "assets/image/keyboard1.png")
	...
	#Mic
	micImg = PhotoImage(file = "assets/image/mic.png")
	...
	#Text Field
	TextFieldImg = PhotoImage(file='assets/image/textField.png')
	...
	#User and Bot Icon
	userIcon = PhotoImage(file="assets/image/avatar/icons/a"+str(ownerPhoto)+".png")
	botIcon = PhotoImage(file="assets/image/assistant2.png")
	...
	#User Photo
	userProfileImg = Image.open("assets/image/avatar/a"+str(ownerPhoto)+".png")
	...
	#Change Photo
	chngPh = ImageTk.PhotoImage(Image.open("assets/image/avatar/changephoto2.png").resize((120, 120)))
	...
	cimg = PhotoImage(file = "assets/image/colorchooser.png")
	...
	root.iconbitmap('assets/image/assistant2.ico')
	raise_frame(root1)
	root.mainloop()