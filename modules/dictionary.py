...
data = json.load(open('assets/dictionary.json', encoding='utf-8'))
...
def getMeaning(word):
	if word in data:
		...
		return word, ["The word doesn't exist in the dictionary."], -1
...
def translate(query):
	...
	if check==1:
		...
		return ["I think you're looking for \"" +word.capitalize()+ '"', "The definition is,\n" + result]
	else:
		return [result, '']
